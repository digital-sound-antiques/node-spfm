# v0.8.0
- YM2608 の ADPCM または SSG で YM2612 の PCM を演奏できるようになりました。
  - PCM のキーオン／オフ位置は VGM データを解析して推定しているため、完璧な再現はできません。ノイズが入ったり、発音が抜けたりする場合があります。
  - オプション `--simulate-ym2612-dac {none|ssg|adpcm|adpcm2}`で、モードを変更できます。
    - `none`: オフ
    - `ssg`: SSG PCM で演奏
    - `adpcm`: ADPCM で演奏
    - `adpcm2`: ADPCM で演奏。PCMの再生周波数が変わったタイミングをキーオフとみなします。`adpcm` でうまく演奏出来ない場合はこちらを指定してください。
